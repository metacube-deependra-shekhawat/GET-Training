...
const saveVehEmpId = () => {
    ...
    if (vehicleDetailsValidated.empId) vehicleDetails.empId = idText;
}
...
const saveVehMake = () => {
    ...
    if (vehicleDetailsValidated.vehMake) vehicleDetails.vehMake = makeText;
}
...
const saveVehModel = () => {
    ...
    if (vehicleDetailsValidated.vehModel) vehicleDetails.vehModel = modelText;
}
...
const saveVehNumber = () => {
    ...
    if (vehicleDetailsValidated.vehNumber) vehicleDetails.vehNumber = numberText;
}
...
const saveVehIdentification = () => {
    ...
    if (vehicleDetailsValidated.vehDesc) vehicleDetails.vehDesc = descText;
}
...
const resetVehForm = () => {
    ...
    for (const key in vehicleDetailsValidated) {
        vehicleDetailsValidated[key] = false;
    }
    const fields = [
        "vehEmpID", "vehicleMake", "vehicleModel", "vehicleNumber", "identification"
    ];
    fields.forEach(id => {
...
const expandPassForm = (param) => {
    if (param === 1) {
        ...
        [...collection].forEach(item => item.checked = true);
...
const getCyclePass = () => {
    ...
    let flag = vehicles.some(vehicle => vehicle.vehId === currVehId && vehicle.vehType === "Cycle");
    if (!flag) {
...
const getBikePass = () => {
    ...
    let flag = vehicles.some(vehicle => vehicle.vehId === currVehId && vehicle.vehType === "MotorBike");
    if (!flag) {
        alert("Please enter a valid vehicle id");
        return;
    }
...
const getCarPass = () => {
    ...
    let flag = vehicles.some(vehicle => vehicle.vehId === currVehId && vehicle.vehType === "Car");
    if (!flag) {
        alert("Please enter a valid vehicle id");
        return;
    }
